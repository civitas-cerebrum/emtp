...
def generate_qna_dataset(
    prompt: str = "You are an expert in {model_expertise}.",
    model_expertise: str = "Software Engineering",
    input_dir: str = "dataset/acquisition/temp/datasources",
    ...
    for filepath in markdown_files:
        ...
        try:
            ...
            print(
                f"DEBUG: Raw JSON response from Ollama for {filename}: {json_response}"
            )  # Added debug print

            # Check if 'response' key exists and is a string, then parse it
            if "response" in json_response:
                print(
                    f"DEBUG: Content of json_response['response'] for {filename}: {json_response['response']}"
                )  # Added debug print
                if isinstance(json_response["response"], str):
                    try:
                        qna = json.loads(json_response["response"])
                        print(
                            f"DEBUG: Parsed qna from string for {filename}: {qna}"
                        )  # Added debug print
                        if "qnaList" in qna:
